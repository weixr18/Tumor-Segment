...
def get_resampled_with_box(input, box, resampled_spacing=[1, 1, 1], l=True):
    ...
    try:
        ...
        resampled_data_ar = moving_resampled_ar[xx.min(
        ):xx.max(), yy.min():yy.max(), zz.min():zz.max()]
    except:
        xx, yy, zz, ee = np.where(moving_resampled_ar > 0)
        resampled_data_ar = moving_resampled_ar[xx.min(
        ):xx.max(), yy.min():yy.max(), zz.min():zz.max(), 0]
    ...
    if len(mask_map_ar.shape) == 4:
        mask_map_ar = mask_map_ar[xx.min():xx.max(
        ), yy.min():yy.max(), zz.min():zz.max(), 0]
    else:
        mask_map_ar = mask_map_ar[xx.min():xx.max(
        ), yy.min():yy.max(), zz.min():zz.max()]
    xx, yy, zz = np.where(mask_map_ar >= 128)
    box_new = [xx.min(), yy.min(), zz.min(), xx.max(),
               yy.max(), zz.max()]  # z,y,x
    ...
    if not must_shape:
        try:
            if len(moving_resampled_ar.shape) == 4:
                ...
                moving_resampled_ar = moving_resampled_ar[xx.min(
                ):xx.max(), yy.min():yy.max(), zz.min():zz.max(), 0]
            else:
...
def get_resampled_with_segs_and_raws(input, segs, othermods, resampled_spacing=[1, 1, 4], l=True, must_shape=None):
    ...
    try:
        ...
        resampled_data = sitk.GetImageFromArray(
            moving_resampled_ar, isVector=True)
    except:
        a = 1
    # resampled_data.CopyInformation(input)
    # resampled_data.SetSpacing(resampled_spacing)
...
def h5viewer(name, moddix, opath):
    ...
    for i in range(ll):
        for j in range(ll):
            I[i*data.shape[1]:(i+1)*data.shape[1], j *
              data.shape[2]:(j+1)*data.shape[2]] = J[z]
            z += 1
            if z == data.shape[0]:
                break
        if z == data.shape[0]:
