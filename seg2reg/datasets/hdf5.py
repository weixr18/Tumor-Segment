...
class AbstractHDF5Dataset(ConfigDataset):
    ...
    def __init__(self, file_path,
        ...
        if mirror_padding is not None:
            if isinstance(mirror_padding, int):
                ...
                assert len(
                    mirror_padding) == 3, f"Invalid mirror_padding: {mirror_padding}"
    ...
    def fetch_and_check(self, input_file_h5, internal_paths):
        ...
        # expand dims if 2d
        def fn(ds): return np.expand_dims(ds, axis=0) if ds.ndim == 2 else ds
    ...
    def do_augmentation(self, array, label=None):
        ...
        if isinstance(label, np.ndarray):
            ...
            augmented, label = spatial_transforms.augment_spatial(
                augmented, seg=label, patch_size=patch_size, patch_center_dist_from_border=[
                    8, 50, 50],
    ...
    def _check_dimensionality(raws, labels):
        ...
        for raw, label in zip(raws, labels):
            assert raw.ndim in [
                3, 4], 'Raw dataset must be 3D (DxHxW) or 4D (CxDxHxW)'
            assert label.ndim in [
                3, 4], 'Label dataset must be 3D (DxHxW) or 4D (CxDxHxW)'
    ...
    def traverse_h5_paths(file_paths):
        ...
        for file_path in file_paths:
            if os.path.isdir(file_path):
                ...
                iters = [glob.glob(os.path.join(file_path, ext))
