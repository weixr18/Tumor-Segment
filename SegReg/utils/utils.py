...
def load_checkpoint(checkpoint_path, model, optimizer=None):
    ...
    try:
        ...
        model.mu = model.mu.cuda()
        model.bias = model.bias.cuda()
    except:
        a = 1
...
class _TensorboardFormatter:
    ...
    def process_batch(self, name, batch, idx_slice):
                ...
                for batch_idx in range(batch.shape[0]):
                    for channel_idx in range(batch.shape[1]):
                        tag = tag_template.format(
                            name, batch_idx, channel_idx, slice_idx[batch_idx])
    ...
    def _embeddings_to_rgb(self, batch):
        ...
        for batch_idx in range(batch.shape[0]):
            ...
            if self.plot_variance:
                cum_explained_variance_img = self._plot_cum_explained_variance(
                    img)
                tagged_images.append(
                    (f'cumulative_explained_variance/batch_{batch_idx}', cum_explained_variance_img))
...
def convert_to_numpy(input, target):
    ...
    assert isinstance(
        target, torch.Tensor), "Expected target to be torch.Tensor"
