    ...
    def __call__(self, input, target):
        ...
        for _input, _target in zip(input, target):
            ...
            for c in range(n_classes):
                ...
                per_channel_iou.append(self._jaccard_index(
                    binary_prediction[c], _target[c]))
...
class AdaptedRandError:
    ...
    def __call__(self, input, target):
        ...
        for _input, _target in zip(input, target):
            ...
            per_channel_arand = [_arand_err(
                _target, channel_segm) for channel_segm in segm]
            logger.info(
...
class BoundaryAdaptedRandError(AdaptedRandError):
    ...
    def __init__(self, thresholds=None, use_last_target=True, input_channel=None, invert_pmaps=True,
        ...
        super().__init__(use_last_target=use_last_target,
                         save_plots=save_plots, plots_dir=plots_dir, **kwargs)
...
class GenericAdaptedRandError(AdaptedRandError):
    def __init__(self, input_channels, thresholds=None, use_last_target=True, invert_channels=None,
        ...
        super().__init__(use_last_target=use_last_target,
                         save_plots=save_plots, plots_dir=plots_dir, **kwargs)
        assert isinstance(input_channels, list) or isinstance(
    ...
    def input_to_segm(self, input):
        ...
        for predictions in input:
            for th in self.thresholds:
                # run connected components on the predicted mask; consider only 1-connectivity
                seg = measure.label((predictions > th).astype(
                    np.uint8), background=0, connectivity=1)
                segs.append(seg)
...
class EmbeddingsAdaptedRandError(AdaptedRandError):
    def __init__(self, min_cluster_size=100, min_samples=None, metric='euclidean', cluster_selection_method='eom',
        ...
        logger.info(
            f'HDBSCAN params: min_cluster_size: {min_cluster_size}, min_samples: {min_samples}')
    ...
    def input_to_segm(self, embeddings):
        ...
        flattened_embeddings = embeddings.reshape(
            embeddings.shape[0], -1).transpose()

        # perform clustering and reshape in order to get the segmentation volume
        start = time.time()
        segm = self.clustering.fit_predict(
            flattened_embeddings).reshape(output_shape)
        logger.info(
            f'Number of clusters found by HDBSCAN: {np.max(segm)}. Duration: {time.time() - start} sec.')
...
class EmbeddingsMeanShiftAdaptedRandError(AdaptedRandError):
    ...
    def input_to_segm(self, embeddings):
        ...
        segm = self.clustering.fit_predict(
            flattened_embeddings).reshape(output_shape)
        logger.info(
            f'Number of clusters found by MeanShift: {np.max(segm)}. Duration: {time.time() - start} sec.')
        return np.expand_dims(segm, axis=0)
...
class BlobsAveragePrecision(GenericAveragePrecision):
    ...
    def __init__(self, thresholds=None, metric='ap', min_instance_size=None, input_channel=-1, **kwargs):
        super().__init__(min_instance_size=min_instance_size,
                         use_last_target=True, metric=metric)
        if thresholds is None:
...
class BoundaryAveragePrecision(GenericAveragePrecision):
    ...
    def input_to_seg(self, input):
        ...
        segs = []
        for th in self.thresholds:
            seg = measure.label(np.logical_not(input > th).astype(
                np.uint8), background=0, connectivity=1)
            segs.append(seg)
        return np.stack(segs)
...
class WithinAngleThreshold:
    ...
    def __call__(self, inputs, targets):
        ...
        for input, target in zip(inputs, targets):
            ...
            within_count += error_radians[error_radians <
                                          self.threshold_radians].numel()
