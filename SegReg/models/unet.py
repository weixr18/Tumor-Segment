...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        if self.multi_head:
            ...
            self.mu = torch.ones(
                (f_maps[0]))*torch.tensor(np.random.randn(f_maps[0])*0.1+1)
            self.mu = self.mu.cuda()
            self.bias = torch.ones(
                (f_maps[0]))*torch.tensor(np.random.rand(f_maps[0]))
            self.bias = self.bias.cuda()
            self.final_conv = nn.ModuleList(
                [nn.Conv3d(f_maps[0], out_channels+1, 1)]*self.head_numbers)
    ...
    def forward(self, x):
        ...
        if self.multi_head:
            ...
            for n_head in range(self.head_numbers):
                t = x*self.mu.reshape((1, -1, 1, 1, 1)) + \
                    self.bias.reshape((1, -1, 1, 1, 1))
                pm = self.final_conv[n_head](t.float())
                p = torch.nn.functional.adaptive_avg_pool3d(
                    pm[:, -1, :, :, :], (1, 1, 1)).view(pm.shape[0], -1)
                pm = pm[:, :-1, :, :, :]
                X.append(pm)
                P.append(p)
...
class CLSNet3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.final_conv(torch.nn.functional.adaptive_avg_pool3d(
            encoders_features).flatten().relu())
        return x
